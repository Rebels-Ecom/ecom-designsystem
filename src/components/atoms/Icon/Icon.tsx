...
/**
 * Canonical list of icon names, in map order. Prefer this over `Object.keys(iconMap)`:
 * Storybook's docgen mistakes the exported `iconMap` object for a component and stamps
 * enumerable `displayName`/`__docgenInfo` properties onto it, which a raw `Object.keys`
 * would surface as bogus icon names (rendering `<Icon icon="__docgenInfo" />` then crashes).
 * Every real name follows the legacy `icon-` convention, so we filter on that to stay safe.
 */
export const iconNames = (Object.keys(iconMap) as IconName[]).filter((name) =>
