...
export interface VerticalVariantsLabels {
...
const defaultVerticalVariantsLabels: Required<VerticalVariantsLabels> = {
...
export interface VerticalVariantsProps {
...
/**
 * A dismissible variant picker laid out as a vertical carousel (organism). Each slide is a
 * {@link ProductVariant} tile; together they form ONE native radio group inside a
...
function VerticalVariants({
  ...
  className,
  ref,
}: VerticalVariantsProps) {
  const t = { ...defaultVerticalVariantsLabels, ...labels }
  ...
  return (
    ...
    >
      {/* Close sits in its own header row (not absolutely overlaid) so it never covers the first
          variant's radio on the right edge. */}
      ...
      <Carousel
        ...
        gap="0.25rem"
        hidePagination
        trackClassName="max-h-96"
        breakpoints={{ sm: { perPage: 2, hideArrows: true }, md: { perPage: 2, perMove: 2 }, lg: { perPage: 2, perMove: 2 } }}
      >
...
export { VerticalVariants, defaultVerticalVariantsLabels }
