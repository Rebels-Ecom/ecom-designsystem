...
const ArticleList = ({ articles, title, swipe, grid }: IArticleList) => {
  ...
  const formatText = (t?: string, maxChar?: number) => {
    ...
    const formattedText =
      maxChar && (extractedRichTextValue?.length ?? 0) > maxChar
        ? extractedRichTextValue?.substring(0, maxChar).concat('...')
        : extractedRichTextValue

    return formattedText
  }

  const useSmallCards = articles.length === 2 || articles.length > 3
  ...
  const articlesToRender = articles.map((article, i) => {
    ...
    return (
      <ArticleCard
        ...
        {...article}
        text={textToDisplay}
        type={useSmallCards ? 'small' : i % 2 === 0 ? 'small' : 'large'}
        equalHeights={useSmallCards}
  ...
  const gridArticlesToRender = articles.map((article, i) => {
    const textToDisplay = formatText(article.text, article.maxChar)
    return <ArticleCard key={i} {...article} text={textToDisplay} type="small" equalHeights />
  })
          ...
          matches && (
            <>
              {swipe ? (
                ...
                <ContentWrapper>
                  <div className={styles.articles}>{articlesToRender}</div>
                </ContentWrapper>
              )}
            </>
          )
          ...
          matches && (
            <ContentWrapper>
              {swipe && articles.length > 3 ? (
                ...
                <div className={styles.articles}>{grid ? gridArticlesToRender : articlesToRender}</div>
              )}
            </ContentWrapper>
          )
        }
