...
import { SocialMediaLinks, TSocialMediaLink } from '../../molecules/social-media-links/social-media-links'
...
const FooterContent = ({logo, address, socialMediaLinks, linkComponent: Link, newsletterId, newsletterPlaceholder }: {logo:INavigationLogo, address:string, socialMediaLinks: any, linkComponent: any, newsletterId:string, newsletterPlaceholder: string }) => {
            ...
            {Array.isArray(list.links) && list.links && (
              <ul className={styles.links}>
                {list.links.map((link:ILink, i:number) => (
                  <li key={`${link.text}-${i}`} className={styles.linkItem}>
                    {link.isExternal ? (
...
const Footer = ({ footerTopBarLinks, logo, address, socialMediaLinks, links = [], linkComponent: Link, newsletterId, newsletterPlaceholder, bottomBarText }: IFooter) => {
  return (
    <>
      <FooterTopBar footerTopBarLinks={footerTopBarLinks} linkComponent={Link}></FooterTopBar>
      <footer className={styles.footer}>
        <FooterContent logo={logo} address={address} socialMediaLinks={socialMediaLinks} linkComponent={Link} newsletterId={newsletterId} newsletterPlaceholder={newsletterPlaceholder} ></FooterContent>
