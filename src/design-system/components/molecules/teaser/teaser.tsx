  ...
  return (
    <div className={`${styles.teaser} ${imagePosition === 'left' ? styles.teaserLeft : styles.teaserRight}`}>
      <div className={cx(styles.imageWrapper, { [styles.imageRound]: imageRound })}>
        ...
        />
        {tag && (
          <Tag {...tag} className={`${styles.tag} ${tagPosition === 'left' ? styles.tagLeft : styles.tagRight}`} />
        )}
      </div>
      <div
        className={`${styles.contentWrapper} ${imagePosition === 'left' ? styles.contentRight : styles.contentLeft}`}
      >
        <h2 className={styles.heading}>{heading}</h2>
        {preamble && <p className={styles.preamble}>{preamble}</p>}
        {richText && <div className={cx(styles.text, imagePosition === 'right' && styles.textRight)}>{richText}</div>}
