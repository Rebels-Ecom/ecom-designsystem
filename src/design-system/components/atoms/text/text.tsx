...
function Text ({ children, align='left', componentType, href, underline=false, weight, className } : IText){
    ...
    if(componentType==='a' && href!=='')
        return(
            <a href={href} className={cx(styles.text, styles.linkText, 'p', weight==='bold' && styles.textBold, underline && styles.textUnderlined, styles[getTextAlignment(align)], className ? className : '')}>
                {children}
            </a>
        )
    return(
        <span className={cx(styles.text, 'p',weight==='bold' && styles.textBold, underline && styles.textUnderlined, styles[getTextAlignment(align)], className ? className : '' )}>
            { children }
        </span>
    )
}
